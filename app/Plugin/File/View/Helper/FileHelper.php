...
private function _parseParams($params){
  ...
  public function getFirstFileUrl($model_object, $category_code, $is_thumbnail_url = FALSE, $order = array()) {
    $object_file_model = new FileModel();
  ...
  public function getThumbnailUrl($relative_path) {
    if (!FileLib::hasThumbnailFile($relative_path)) {
      $file = $this->getFileUrl($relative_path);
      if(FileLib::isExistFile($relative_path)){
        return $this->getFileUrl($relative_path);
      }
      return NULL;
    }
...
END;
  ...
  public function getAjaxUploadInFolderForm($folderId, $eId, $is_multiple_upload, $add_parameters, $after_complete_one, $after_complete_all, $jsCallbackFunction_after_add_one, $allowedExtensions) {
    $allowedExtensions = $allowedExtensions ? "['".implode("','", $allowedExtensions)."']" : "[]";
    $webroot_url = Router::url("/") . "file";
    $lastDir = 'Object__' . uniqid('');
...
END;
    ...
    $html_forms .=<<<END
      <script>
        createUploader('$webroot_url/ajax_upload/$folderId', '$lastDir', $is_multiple_upload, '$eId', $add_parameters, $after_complete_one, $after_complete_all, "$jsCallbackFunction_after_add_one", $allowedExtensions);
      </script>
END;
  ...
  public function printLinkPath($baseUrl = '/', $pathArray = array()){
    $path = '';
    foreach ($pathArray as $pathArr) {
      $path.= '/'. sprintf("<a href='%s'>%s</a>", $baseUrl . '/' .$pathArr[0], $pathArr[1]);
    }
